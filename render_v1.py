...
def merge_svgs_to_layers(layers, output_path):
...
def create_outer_outline_smooth(image_path, output_path, outline_color="black", outline_size=10, smooth_factor=10):
    # Load ảnh PNG và chuyển thành RGBA
    image = Image.open(image_path).convert("RGBA")
    img_array = np.array(image)
    ...
    # Tạo mask nhị phân (1 = vùng có đối tượng, 0 = vùng trong suốt)
    binary_mask = np.uint8((alpha_channel > 0) * 255)

    # Mở rộng contour bằng Morphological Dilation (tạo outline cách đều)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (outline_size, outline_size))
    dilated = cv2.dilate(binary_mask, kernel, iterations=1)

    # Làm mượt bằng GaussianBlur để loại bỏ chi tiết nhỏ
    blurred = cv2.GaussianBlur(dilated, (smooth_factor * 2 + 1, smooth_factor * 2 + 1), 0)
    ...
    # Vẽ outline với đường mượt hơn
    for contour in contours:
        # Giảm số lượng điểm bằng approxPolyDP
        epsilon = 0.01 * cv2.arcLength(contour, True)  # Độ chính xác, số càng lớn thì càng ít điểm
        smoothed_contour = cv2.approxPolyDP(contour, epsilon, True)

        points = [(int(pt[0][0]), int(pt[0][1])) for pt in smoothed_contour]
        if len(points) > 1:
            path_data = "M " + " L ".join(f"{x},{y}" for x, y in points) + " Z"
            dwg.add(dwg.path(d=path_data, fill="none", stroke=outline_color, stroke_width="2"))
    ...
    # Lưu file SVG
    dwg.save()
    print(f"Smooth outer outline saved to {output_path}")
...
if __name__ == "__main__":
    ...
    # create_outer_outline_smooth(input_image_path, output_outline_radius, "black", 50, 100)
    ...
    merge_svgs_to_layers([output_black, output_outline_radius], output_merged_svg)
